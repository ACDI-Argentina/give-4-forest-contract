...
async function main() {
  ...
  console.log(`MilestoneLib deploy`);
  const milestoneLib = await MilestoneLib.new({ from: deployer });
  const milestoneLibAddress = milestoneLib.address;
  console.log(` - MilestoneLib address: ${milestoneLibAddress}`);
